...
用途：用户无需充值大模型，即可用三个真实风格的案例工厂（A企业/B企业/C企业）
...
CASES = {
    "A企业": {
        ...
        "qdata": {
            "entName": "A企业", "mainBiz": "高端装备制造/通用设备",
            "industry": "通用设备制造业", "employees": "3650人", "revenue": 528000,
            "factoryName": "A企业核心制造基地智能工厂", "totalInvest": 32000, "integrator": "某工业互联网平台服务商",
    ...
    "B企业": {
        ...
        "level": "先进级",
        "industry": "通用机械零部件",
        "submission": f"{EXPERT_PACK}/1784556664710279767-sample2_不合格_B企业.txt",
        "scores": {"智能制造成熟度": 2, "信息基础建设水平": 8, "技术应用先进性": 3,
                   "解决方案示范性和可推广性": 2, "经济社会效益情况": 5, "后续实施计划": 8},
        "form": {
            "企业名称": "B企业", "所属行业": "通用机械零部件",
            "年营业收入": 800, "员工人数": 45,
        ...
        "qdata": {
            "entName": "B企业", "mainBiz": "通用机械零部件",
            "industry": "通用设备制造业", "employees": "45人", "revenue": 800,
            "factoryName": "B企业数字化改造项目", "totalInvest": 30, "integrator": "",
    ...
    "C企业": {
        ...
        "submission": f"{EXPERT_PACK}/1784556664798354131-sample3_基础级_C企业.txt",
        "scores": {"智能制造成熟度": 10, "信息基础建设水平": 22, "技术应用先进性": 14,
                   "解决方案示范性和可推广性": 7, "经济社会效益情况": 8, "后续实施计划": 9},
        "form": {
            "企业名称": "C企业", "所属行业": "计算机、通信和其他电子设备制造业",
            "年营业收入": 8500, "员工人数": 280,
            "是否高新技术企业": "是", "是否专精特新企业": "否",
            "是否已有数字化车间/智能工厂认证": "否", "近三年有无安全/环保重大事故": "无",
        ...
        "qdata": {
            "entName": "C企业", "mainBiz": "SMT贴片/组装",
            "industry": "计算机、通信和其他电子设备制造业", "employees": "280人", "revenue": 8500,
            "factoryName": "C企业智能制造基地", "totalInvest": 1200, "integrator": "某本地集成商",
...
def _aux_template_search(text: str) -> str:
    """F4：范本片段检索。"""
    templates = [
        ("A企业·卓越级", "9大章结构", "封面→基本信息→场景建设→成效→投资预算→进度→预期效益→保障措施→风险分析", "适用于大型装备制造/营收≥2亿/成熟度三级以上"),
        ("A企业·先进级", "7大章结构", "封面→基本信息→场景建设→成效→投资预算→进度→预期效益→保障措施", "适用于中型制造/营收5000万以上/MES已部署"),
        ("某物流装备企业·先进级", "7大章结构", "封面→基本信息→场景建设→成效→投资预算→进度→预期效益→保障措施", "适用于物流/仓储行业参考"),
    ]
    lines = ["### 📚 范本检索（离线模拟）\n"]
    matched = []
    ...
    if matched:
        ...
        lines.append("- 未检索到匹配范本。可用关键词：`A企业` / `某物流装备企业` / `卓越级` / `先进级` / `装备` / `电子`\n")
    else:
...
def _aux_knowledge_qa(text: str, case_form: dict = None) -> str:
    ...
    intent_rules = [
        ...
        ("指标门槛", ["门槛", "要求", "指标", "数控化率", "联网率", "营收", "成熟度"],
         ...
         f"| 成熟度 | ≥二级 | ≥二级 | ≥三级 | ≥四级 |\n"),
        ("案例参考", ["案例", "别人", "例子", "通过", "成功"],
         "脱敏案例（知识库）：\n- **A企业**（装备制造）→ 先进级通过（MES+PLM+SCADA 全覆盖）\n- **C企业**（电子制造）→ 基础级通过（SMT 产线+MES+WMS）\n"
         "- **B企业**（零部件）→ 先进级未通过（缺 MES/联网率低）\n"
         "更多案例可在「②政策问答」中查询。"),
    ]
